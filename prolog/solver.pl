...
transpose([FIRST_ROW|OTHER_ROWS], COLUMNS) :-
  transpose(FIRST_ROW, [FIRST_ROW|OTHER_ROWS], COLUMNS).

transpose([],_,[]).
/*Isolate the tail of the first row, all rows, and the first column*/
transpose([_|REST_FR],ALL_ROWS,[COL_1|OTHER_COLS]) :-
  ...
  transpose(REST_FR, RESULT, OTHER_COLS).
